...
\textcolor{red}{MIDI note number calculations.}
...
To calculate the MIDI note number $m$ corresponding to the closest note to frequency $f$, we can use the number of semitones from \note{C}{0} $n_s$ and the fact that \note{C}{0} has note number $12$:
% \[ m = n_s + 12 \]
\begin{align*}
    m = n_s^{C_0} + 12
\end{align*}
...
\subsection{Parallel tuned Fourier transforms}
